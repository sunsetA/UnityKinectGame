...
//			return new int[0];
    ...
    protected void CheckPoseComplete(ref GestureData gestureData, float timestamp, Vector3 jointPos, bool isInPose)
    {
        if (isInPose)
        {
            float timeLeft = timestamp - gestureData.timestamp;
	...
	public virtual void CheckForGesture(long userId, ref GestureData gestureData, float timestamp, ref Vector3[] jointsPos, ref bool[] jointsTracked)
	{
		...
		switch(gestureData.gesture)
		{
            ...
            // check for ClapHands
            case Gestures.ClapHands:
                if (jointsTracked[leftHandIndex] && jointsTracked[rightHandIndex] &&
                     Vector3.Distance(jointsPos[leftHandIndex], jointsPos[rightHandIndex]) < 0.03f &&
                    ...
                    (jointsPos[rightHandIndex].y - jointsPos[rightShoulderIndex].y) < 0.1f)
                {
                    ...
                    Vector3 _jointPos = jointsPos[gestureData.joint];
                    CheckPoseComplete(ref gestureData, timestamp, _jointPos, true);
                    //SetGestureJoint(ref gestureData, timestamp, rightHandIndex, jointsPos[rightHandIndex]);
                }
                break;

            // check for RaiseLeftRightHand
            case Gestures.RaiseLeftRightHand:
                switch (gestureData.state)
                {
                    case 0:  // gesture detection
                        if (jointsTracked[leftHandIndex] && jointsTracked[rightHandIndex] && jointsTracked[rightShoulderIndex] &&
                    ...
                    case 1:  // gesture complete
                        ...
                        Vector3 jointPos = jointsPos[gestureData.joint];
                        CheckPoseComplete(ref gestureData, timestamp, jointPos, isInPose,3f);
                        break;
                }
                break;
